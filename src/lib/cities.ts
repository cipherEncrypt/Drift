...
export function cityLabel(slug: string | null | undefined): string | null {
  return cityBySlug(slug)?.name ?? null
}
...
export function pinFromProfile(profile: {
  city?: string | null
  cityLat?: number | null
  cityLng?: number | null
  ...
  if (
    profile &&
    typeof profile.cityLat === 'number' &&
    typeof profile.cityLng === 'number' &&
    Number.isFinite(profile.cityLat) &&
    Number.isFinite(profile.cityLng)
  ) {
    return [profile.cityLat, profile.cityLng]
  }

  const listed = cityBySlug(profile?.city)
  if (listed) return [listed.lat, listed.lng]
